...
# Usage: curl -fsSL https://raw.githubusercontent.com/thedavidweng/canvas-cli/main/install.sh | sh

REPO="thedavidweng/canvas-cli"
BINARY="canvas"
CASK="canvas"
...
install_binary() {
  ...
  bin_dir="${CANVAS_INSTALL_DIR:-$HOME/.local/bin}"
  mkdir -p "$bin_dir"
...
# --- Uninstall helper ---
uninstall_brew() {
  step "Uninstalling Homebrew-managed canvas-cli"
...
case "${1:-}" in
  uninstall)
    if has_brew && brew list --cask "$CASK" >/dev/null 2>&1; then
      ...
      step "Uninstalling legacy Homebrew formula for canvas-cli"
      brew uninstall --formula "$BINARY"
    else
      uninstall_binary
    fi
    step "Uninstalled. You may also remove canvas-cli config from ~/.config/canvas-cli/"
    exit 0
    ;;
  --help|-h)
    cat <<EOF
...
Installs canvas-cli. Prefers Homebrew Cask if available, otherwise
downloads the binary to ~/.local/bin.

Environment:
  CANVAS_INSTALL_DIR  Directory for binary (default: ~/.local/bin)

Options:
  uninstall    Remove canvas-cli
...
step "Installing canvas-cli ($platform_label)"
...
printf '\n'
step "Run 'canvas auth login' to get started."
step "Run 'canvas --help' to see available commands."
