# canvas-cli installer for Windows
# Usage: powershell -ExecutionPolicy ByPass -c "irm https://raw.githubusercontent.com/thedavidweng/canvas-cli/main/install.ps1 | iex"
...
function Install-Canvas {
    Step "Installing canvas-cli ($platformLabel)"
    ...
    $installDir = if ($env:CANVAS_INSTALL_DIR) { $env:CANVAS_INSTALL_DIR } else {
        Join-Path $env:LOCALAPPDATA "canvas-cli\bin"
    }

    $tmpDir = Join-Path $env:TEMP "canvas-cli-install-$([guid]::NewGuid().ToString('N').Substring(0,8))"
...
function Uninstall-Canvas {
    ...
    Step "Uninstalled. You may also remove canvas-cli config from $env:APPDATA\canvas-cli\"
}
